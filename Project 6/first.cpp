...
#ifndef MIN_SIZE
#define MIN_SIZE 512*512
#endif 

#ifndef MAX_SIZE
#define MAX_SIZE POINTS*MIN_SIZE
#endif
...
main( int argc, char *argv[ ] )
{
	...
	for (int i = MIN_SIZE; i <= MAX_SIZE; i = i + (MAX_SIZE - MIN_SIZE) / (POINTS-1)) {
	...
	int vals_that_work[] = { 8,10,16,20,32,40,64,80,100,128,160,200,256,320,400,512 };
	file.open("local.csv");
	for (int i = 0; i < 18; i++) {
		m = ArrayMult(MIN_SIZE*50, vals_that_work[i]);
		ma = ArrayMultAdd(MIN_SIZE * 50, vals_that_work[i]);
		if (m != -1 && ma != -1 && mr != -1) {
